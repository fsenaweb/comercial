...
| Tela trava com `The stream or file ".../storage/logs/laravel.log" could not be opened in append mode: Permission denied` (às vezes acompanhado de `mkdir(): Permission denied` na tela de Backup) | `backend/storage` é bind mount — o dono real é o do **host**, não o que o `Dockerfile` define pra imagem. Se em algum momento a pasta foi tocada por outro usuário (`docker compose exec -u root ...`, restauração de backup, rebuild com `UID`/`GID` diferente do dono atual), o `www-data` do container deixa de conseguir escrever ali — nem log, nem backup. `EnsureBackupDirectoryPermissionsCommand` sozinho não resolve esse caso: ele só faz `chmod`, e quem não é dono do arquivo não consegue nem `chmod` sem ser root. Achado real na loja do cliente (2026-08-05) | Corrigido: `deploy.sh`/`deploy.bat` agora rodam `docker compose exec -u root php-fpm chown -R www-data:www-data storage bootstrap/cache` a cada deploy, incondicionalmente — não só quando alguém percebe o erro na tela. Pra corrigir manualmente sem esperar o próximo deploy, rodar esse mesmo comando (+ `chmod -R ug+rwX storage bootstrap/cache`) direto na máquina da loja |
...
docker compose up -d
docker compose exec -u root php-fpm chown -R www-data:www-data storage bootstrap/cache   # ver "Armadilhas conhecidas"
...
- **Achado real (2026-07-23, loja do cliente):** `storage/app/backup/` criado com permissão `0700` (padrão do Flysystem antes do disco `backups` ganhar `visibility => public` em `config/filesystems.php`) — em Windows/Docker Desktop (bind mount via WSL2), esse bit fica inconsistente entre execuções, e a tela de Backup passava a dar "Permission denied" ao listar o próprio diretório. Corrigido em duas camadas: `visibility => public` evita o problema em diretórios novos; `EnsureBackupDirectoryIsAccessibleAction` corrige (recursivamente, 0755/0644) diretórios que já existiam quebrados — roda tanto a cada acesso à tela de Backup quanto como passo explícito do deploy (`php artisan backups:ensure-directory-permissions`, chamado por `deploy.sh`/`deploy.bat` logo após `storage:link`), então rodar o deploy já garante a correção sem depender de alguém abrir a tela primeiro.

...
- **Setup único, por ambiente** (feito uma vez pelo desenvolvedor/mantenedor, não pelo lojista):
  ...
  6. **Publicar o app** (na interface atual, "Google Auth Platform" → aba **Público-alvo** → botão "Publicar app"; muda o status de "Testando" para "Em produção"). **Passo obrigatório, não opcional:** enquanto o app está "Testando", só e-mails cadastrados como "usuário de teste" conseguem autorizar, **e o refresh token expira sozinho a cada 7 dias** — inviável para um backup diário automático. Publicando (sem precisar da verificação formal da Google, que só é exigida acima de 10 domínios, logotipo ou escopos restritos/confidenciais — não é o caso aqui), o token deixa de expirar sozinho; o preço é que a primeira autorização mostra um aviso "O Google não verificou este app" — normal para apps internos não verificados, resolve-se clicando em "Avançado" → "Acessar (nome do app) (não seguro)".
     - **Armadilha real (achada num deploy na loja do cliente, 2026-09-23):** o botão "Publicar app" vem desabilitado até completar a aba **Branding** com "Nome do app", "E-mail para suporte do usuário" e "E-mail de contato do desenvolvedor" (obrigatórios) — e a Google passou a exigir também URLs de **"Página inicial do aplicativo"** e **"Link da Política de Privacidade"**, que por sua vez precisam ter o domínio cadastrado em "Domínios autorizados" (fluxo de verificação via Google Search Console, método de arquivo HTML, resolvido automaticamente ao clicar em "+ Adicionar domínio"). Como o sistema não tem domínio público (é on-premise/LAN), essas duas páginas foram publicadas via GitHub Pages num repositório à parte, só com esse propósito: [`fsenaweb/sistema-comercial-paginas`](https://github.com/fsenaweb/sistema-comercial-paginas) → `https://fsenaweb.github.io/sistema-comercial-paginas/` (página inicial) e `https://fsenaweb.github.io/sistema-comercial-paginas/privacy.html` (política de privacidade, descrevendo o uso do escopo `drive.file`).

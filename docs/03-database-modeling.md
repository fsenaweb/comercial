...
- **users**: id, name, email, password, `role` (enum: `admin`, `cashier`, `seller`), commission_percent (nullable — comissão do vendedor), active (boolean), soft deletes, timestamps.
  - Não existe tabela `sellers` separada: o vendedor **é** o usuário logado (decisão fechada).
  - Autorização por papel via Policies do Laravel, não por tabela de permissões.

## 2. Configuração da Loja
- **store_settings**: registro único (id fixo = 1). name, cnpj (nullable), address, phone, logo_path (nullable), `require_seller_on_sale` (boolean — se a seleção de vendedor no PDV é obrigatória), `auto_open_cash_register` (boolean — se o caixa abre automaticamente ao iniciar o expediente).
  - Essa tabela existia apenas como item de roadmap no doc 06 ("config da loja, 1 registro") mas nunca havia sido modelada — lacuna fechada aqui.
...
## 7. Regras de negócio associadas ao modelo

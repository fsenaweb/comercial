...
- [ ] API CRUD (Model, Migration, Controller, Form Requests, Resources, Policy) para `categories`, `subcategories`, `brands`, `units`.
- [ ] API CRUD para `products` + `product_variations` (SKU), incluindo registro do estoque inicial via `stock_movements` (nunca gravar `current_quantity` direto) — a migration de `stock_movements` entra aqui por dependência.
- [ ] API CRUD para `customers` e `suppliers`.
- [ ] Telas Nuxt de listagem e formulário para todas as entidades acima (componentes de tabela/formulário reutilizáveis em `components/ui/`).
- [ ] Máscaras no frontend: moeda, CPF/CNPJ, CEP (composables testáveis).
- [ ] Testes: feature tests de CRUD (create/update/delete/list + validação de Form Request + autorização por papel) para cada entidade acima; Vitest para os composables de máscara.
...
- [ ] CRUD de usuários/vendedores no back-office (hoje o admin inicial vem do seeder; novos usuários exigem tela própria — encaixar na Sprint 1 ou 2).

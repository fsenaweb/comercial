...
9. [Instalação na Loja (Windows 10)](./docs/10-instalacao-loja.md) — tutorial passo a passo para instalar do zero no PC final da loja (pré-requisitos, clone, `.env`, subida inicial da stack, impressão térmica, atualizações via `deploy.bat`). Distinto do runbook de desenvolvimento em `07-dev-environment.md`.
10. [Migração do Sistema Legado](./docs/11-migracao-sistema-legado.md) — levantamento do schema Firebird do ERP antigo e arquitetura de importação (extração → CSV → `php artisan legacy:import`). Escopo final definido pelo cliente: **só produtos e clientes** (cadastros), sem histórico de vendas/estoque/caixa/crediário. **Documento vivo, trabalho em andamento** — consultar antes de continuar essa migração em qualquer sessão futura.
